...
public class UtilityClass extends BasePackage {
	...
	public static void SeekBarTest() {
		...
		new TouchAction((PerformsTouchActions) Const.driver).longPress(PointOption.point(startX, yAxis))
	...
	public static List<MobileElement> getGroupName() {
		return Const.driver.findElements(By.xpath("//android.widget.TextView[@text='NYT Experiments']"));
	...
	public static List<MobileElement> getSectionListSectionName() {
		return Const.driver.findElements(By.xpath("//android.widget.TextView[@text='Technology']"));

	}
	
	public static void scrollSectionListVertical() {
	...
	public static void scrollUp() {
		try {
			...
			// new TouchAction((PerformsTouchActions)
			// driver).press(PointOption.point(startx, starty))
			// .waitAction(WaitOptions.waitOptions(Duration.ofSeconds(3))).moveTo(PointOption.point(endx,
			// endy))
			// .release().perform();
